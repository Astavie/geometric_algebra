...
impl ppga2d::Point {
    pub fn translate(self, dir: ppga2d::Direction) -> Self {
        self + dir * self[0]
    }
}

impl ppga3d::Point {
    pub fn translate(self, dir: ppga3d::Direction) -> Self {
        self + dir * self[0]
    }
}

impl ppga2d::Origin {
    pub fn translate(self, dir: ppga2d::Direction) -> ppga2d::Point {
        ppga2d::Point::from_raw(self[0], dir[0] * self[0], dir[1] * self[0])
    }
}

impl ppga3d::Origin {
    pub fn translate(self, dir: ppga3d::Direction) -> ppga3d::Point {
        ppga3d::Point::from_raw(
            self[0],
            dir[0] * self[0],
            dir[1] * self[0],
            dir[2] * self[0],
...
impl ppga2d::Direction {
    pub const fn new(xo: f32, yo: f32) -> Self {
        Self::from_raw(yo, xo)
    }
}
...
impl ppga2d::Translator {
    pub fn ln(self) -> ppga2d::Direction {
        Into::<ppga2d::Direction>::into(self) / self[0]
    }
    pub fn powf(self, exponent: f32) -> Self {
...
mod tests {
    ...
    fn it_works() {
        ...
        const FRAC: f32 = 2.0 / (POINTS as f32);

        let center = ppga2d::ORIGIN;
        let radius = 1.0;

        let rotor = (PI / 2.0 * center).exp();
        let on_circle = center.translate(ppga2d::UP * radius);

        let points = (0..POINTS)
            .map(|p| rotor.powf(p as f32 * FRAC).sandwich(on_circle))
            .collect::<Vec<_>>();
    ...
    fn vectorspace() {
        ...
        let a = Vector::from_raw(1.0, 3.0, 2.0);
        let b = Vector::from_raw(6.0, 2.0, 5.0);
    ...
    fn spacetime() {
        use sta::*;
        const G0: SpacetimeVector = SpacetimeVector::from_raw(1.0, 0.0, 0.0, 0.0);

        let v = SpacetimeVector::from_raw(1.0, 4.0, 3.0, 6.0);
        let split = v * G0;
        println!("{:?}", split);
        assert!(false);
    }
